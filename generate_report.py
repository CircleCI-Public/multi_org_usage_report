...
# Function to create a usage export job with all organization IDs
def create_usage_export_job(org_id, circle_token, start_date, end_date, shared_org_ids):
    ...
    # Ensure all org IDs are included and formatted correctly
    org_id_list = []
    for org in shared_org_ids:
        if isinstance(org, dict) and 'id' in org:
            org_id_list.append(org['id'])
        elif isinstance(org, str):
            org_id_list.append(org)
    
    # Always include the primary org ID if it's not already in the list
    if org_id not in org_id_list:
        org_id_list.append(org_id)
    
    # Prepare the payload with all organization IDs
    data = {
        "start": start_date,
        "end": end_date,
        "shared_org_ids": org_id_list
    }
    
    print(f"Creating export job for timeframe: {start_date} to {end_date}")
    print(f"Including {len(org_id_list)} organizations in the report")
    ...
    print(f"Export job created successfully: {json.dumps(job_data, indent=2)}")
    return job_data.get('usage_export_job_id')
...
# Process a single time frame for all organizations in one request
def process_specific_timeframe(orgs, start_date, end_date):
    print(f"\nProcessing timeframe: {start_date} to {end_date} for all organizations")
    
    if not orgs:
        print("No valid organizations found.")
        return None
    
    # Create the usage export job with all organization IDs in a single request
    job_id = create_usage_export_job(PRIMARY_ORG_ID, CIRCLE_TOKEN, start_date, end_date, orgs)
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Process the specific timeframe with all orgs in a single request
